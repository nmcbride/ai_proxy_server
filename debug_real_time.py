        ...
        try:
            async with client.stream(
                ...
                async for chunk_bytes in response.aiter_bytes(chunk_size=1):
                    ...
                    if chunk_bytes:
                        ...
                        if chunk_count % 100 == 0:
                            print(
                                f"[{elapsed:.4f}s] Received {chunk_count} bytes",
                                flush=True,
                            )
                ...
                if len(chunk_times) > 1:
                    ...
                    avg_chunk_interval = (
                        streaming_duration / len(chunk_times)
                        if len(chunk_times) > 1
                        else 0
                    )

                    print("📊 Raw HTTP Streaming Analysis:")
                    ...
                    print(
                        f"  Average interval: {avg_chunk_interval * 1000:.2f}ms per byte"
                    )
                    ...
                    if streaming_duration > 0.1:  # If streaming took more than 100ms
                        ...
                        print(
                            "  🚨 DETECTED: Buffered response (all bytes arrived at once)"
                        )
