...
class RequestProfiler:
    ...
    def start_timing(self, name: str, **metadata) -> TimingEntry:
    ...
    @asynccontextmanager
    async def time_phase(self, name: str, **metadata):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_phase_time = sum(t.duration_ms for t in finished_timings) if finished_timings else 0
        ...
        return {
            ...
            "breakdown": {
                t.name: t.duration_ms for t in finished_timings
            }
        }
    ...
    def get_slowest_phases(self, limit: int = 5) -> List[TimingEntry]:
        ...
        finished_timings = [t for t in self.timings if t.duration_ms is not None]
        return sorted(finished_timings, key=lambda x: x.duration_ms or 0, reverse=True)[:limit]
...
def cleanup_old_profilers(max_age_seconds: int = 300):
...
async def time_json_operation(profiler: RequestProfiler, operation: str, data_size: int = None):
    """Time JSON parsing/serialization operations"""
    metadata = {"operation": operation}
    if data_size:
        metadata["data_size_bytes"] = data_size
    return profiler.time_phase(f"json_{operation}", **metadata)


async def time_network_request(profiler: RequestProfiler, method: str, url: str):
    """Time network requests"""
    return profiler.time_phase(
        "network_request", 
        method=method, 
        url=url
    )
...
async def time_mcp_operation(profiler: RequestProfiler, operation: str, server: str = None, tool: str = None):
    """Time MCP operations"""
    metadata = {"operation": operation}
    if server:
    ...
    if tool:
        metadata["tool"] = tool
    return profiler.time_phase(f"mcp_{operation}", **metadata)


async def time_plugin_execution(profiler: RequestProfiler, plugin_type: str, plugin_count: int = None):
    """Time plugin execution"""
    metadata = {"plugin_type": plugin_type}
    if plugin_count:
        metadata["plugin_count"] = plugin_count
    return profiler.time_phase(f"plugin_{plugin_type}", **metadata) 