...
class MCPServerConnection:
        ...
        try:
            ...
            if transport_type == "stdio":
                ...
                logger.error("Unsupported transport type", 
                           server=self.name, transport=transport_type)
                return False
    ...
    async def _load_capabilities(self):
        ...
        try:
            ...
            logger.error("Failed to load capabilities from MCP server", 
                        server=self.name, error=str(e))
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            logger.error("Tool call failed", 
                        server=self.name, tool=tool_name, error=str(e))
            raise
        ...
        for server_name, config in server_configs.items():
            try:
                ...
                if success:
                    ...
                    logger.warning("Failed to connect to MCP server", server=server_name)
            
            except Exception as e:
                logger.error("Error initializing MCP server", 
