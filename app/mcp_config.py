...
class MCPConfig:
    ...
    def load_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.config_path.exists():
            try:
                ...
                logger.info("MCP config loaded from file", 
        ...
        for key, value in os.environ.items():
            if key.startswith('MCP_SERVER_'):
                ...
                try:
                    ...
                    logger.info("MCP server config loaded from env", 
                               server=server_name)
                    ...
                    if parts:
                        ...
                        logger.info("MCP server config loaded from env (simple)", 
                                   server=server_name)
    ...
    def create_example_config(self):
        ...
        example_config = {
            'mcp_servers': {
                ...
                'home_assistant': {
                    ...
                    'args': ['--url', 'http://localhost:8123', '--token', 'your_ha_token'],
                    'env': {
        ...
        try:
            ...
            logger.error("Failed to create example MCP config", 
                        path=str(self.config_path), error=str(e))
            return False
    ...
    def save_config(self):
        ...
        try:
            ...
            logger.error("Failed to save MCP config", 
                        path=str(self.config_path), error=str(e))
            return False
